...
namespace tool {
    ...
    public:
        ...
        inline GeneralHeContext(bool device, SchemeType scheme, size_t poly_modulus_degree, size_t log_t, vector<size_t> log_qi, 
            bool expand_mod_chain, uint64_t seed, double input_max = 0, double scale = 0, double tolerance = 1e-4,
            bool to_device_after_keygeneration = false, bool use_special_prime_for_encryption = false,
            MemoryPoolHandle pool = MemoryPool::GlobalPool()
        ): GeneralHeContext(GeneralHeContextParameters(device, scheme, poly_modulus_degree, log_t, log_qi, expand_mod_chain, seed, input_max, scale, tolerance, to_device_after_keygeneration, use_special_prime_for_encryption, pool)) {}
