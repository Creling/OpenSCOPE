...
from noisy_activation_utils import replace_relu_with_layer_aware_noisy_gelu, layer_noise_map_db_fu5
...
# Initialize wandb
wandb.init(
    project="model-stealing-resnet_fixsamples",
    ...
    config={
        ...
        "learning_rate": 1e-3,
        "target_snr_db": -1,
        "target_model_path": "resnet18_organamnist_gelu_noisygelu_db_-5.pth",
        "noise_std": "layer_noise_map_db_fu5",
        # "noise_std": None,
        "target_snr_db": None,
...
queryset = torch.utils.data.Subset(testset, range(len(testset) // 2))  
valset = torch.utils.data.Subset(testset, range(len(testset) // 2, len(testset))) 
...
# ----------------------------
# ----------------------------
approx_model = resnet18(pretrained=False)
...
print(f"目标模型在验证集上的准确率: {target_acc:.2f}%")
...
for epoch in range(num_epochs):
    ...
    if approx_acc > best_acc:
        best_acc = approx_acc
        best_epoch = epoch + 1
        best_model_path = "resnet18_organamnist_approx_best.pth"
        torch.save(approx_model.state_dict(), best_model_path)
        # wandb.save(best_model_path)
        print(f"保存最佳模型，准确率: {best_acc:.2f}%")

checkpoint_path = f"resnet18_organamnist_{config.strategy}_noisestd_{config.noise_std}_sample_{config.sample_per_cls}_epoch_{num_epochs}.pth"
torch.save(approx_model.state_dict(), checkpoint_path)

for hook in hooks:
    hook.remove()

approx_model.load_state_dict(torch.load("resnet18_organamnist_approx_best.pth"))
